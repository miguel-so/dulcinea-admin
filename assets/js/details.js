(function () {
  ...
  const descriptionElement = document.querySelector('[data-artwork-description]');
  ...
  const setFeedback = (message, tone = 'info') => {
    ...
    feedbackElement.classList.remove('text-success', 'text-danger', 'text-info');
  ...
  const renderCarousel = (images) => {
    ...
    images.forEach((imageUrl, index) => {
      ...
      indicator.className = `li${index + 1}`;
  ...
  const normaliseDetails = (artwork) => {
    ...
    if (typeof artwork?.note === 'string' && artwork.note.trim()) {
      return artwork.note.split(/\r?\n/).filter(Boolean);
    }
  ...
  const populatePage = (artwork) => {
    ...
    if (descriptionElement) {
      descriptionElement.textContent = artwork.note || artwork.description || 'No description available.';
    }

    if (contactButton) {
      contactButton.href = `contact-us.html?artworkId=${encodeURIComponent(artwork.id)}`;
      contactButton.removeAttribute('aria-disabled');
    }

    const thumbnailUrl = artwork.thumbnail ? window.App.resolveArtworkAsset(artwork.thumbnail) : '';
    const galleryImages = Array.isArray(artwork.images) ? artwork.images : [];

    const uniqueImages = [thumbnailUrl, ...galleryImages.map((fileName) => window.App.resolveArtworkAsset(fileName))]
      .filter(Boolean)
  ...
  if (!artworkId) {
    setFeedback('No artwork selected. Please return to the gallery and choose an artwork to view.', 'danger');
    if (contactButton) {
  ...
  const loadArtwork = async () => {
    try {
      ...
      const data = await window.App.request(`/api/artworks/${encodeURIComponent(artworkId)}`);
      ...
      setFeedback('We were unable to load this artwork. Please try again later.', 'danger');
      if (contactButton) {
        contactButton.classList.add('disabled');
        contactButton.setAttribute('aria-disabled', 'true');
      }
