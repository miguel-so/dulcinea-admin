(function () {
  ...
  const updateFavoriteButton = (button, artworkId) => {
    ...
    button.innerHTML = `<span class="mobi-mbri mobi-mbri-hearth mbr-iconfont mbr-iconfont-btn"></span>${
  ...
  const createArtworkCard = (artwork) => {
    ...
    description.textContent = artwork.notes || "No description available";
  ...
  const loadArtworks = async () => {
    try {
      ...
      setFeedback(
        "We ran into a problem while loading artworks. Please try again shortly.",
        { hidden: false, tone: "danger" }
      );
    }
  };
  ...
  grid.addEventListener("click", (event) => {
    ...
    favoriteButton.blur();
    setFeedback(
      updatedState ? "Added to favorites." : "Removed from favorites.",
      { hidden: false, tone: "success" }
    );
