(function () {
  ...
  if (contextElement) {
    if (artworkId) {
      ...
      contextElement.textContent = `We will share this message with the artist for artwork #${artworkId}.`;
    } else {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      setAlert(successAlert, 'Thanks for reaching out! The artist will be notified shortly.');
      form.reset();
    } catch (error) {
      console.error('Failed to submit contact form', error);
      setAlert(errorAlert, 'We were unable to send your message. Please try again later.');
    } finally {
      setLoading(false);
    }
  });
